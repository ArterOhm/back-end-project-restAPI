	...
	return &config{
		...
		jwt: &jwt{
			...
			accessExpiresAt: func() int {
				...
				if err != nil {
					log.Fatalf("load access expiresAt failed: %v", err)
				}
				return a
			}(),
			refreshExpiresAt: func() int {
				a, err := strconv.Atoi(envMap["JWT_REFRESH_EXPIRES"])
				if err != nil {
					log.Fatalf("load refresh expiresAt failed: %v", err)
				}
				return a
			}(),
		},
...
func (d *db) Url() string {
	return fmt.Sprintf(
		"host=%s post=%d user=%s password=%s dbname=%s sslmode=%s",
		d.host,
		d.port,
		d.username,
		d.password,
